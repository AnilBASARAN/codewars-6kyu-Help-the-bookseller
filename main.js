function stockList(listOfArt, listOfCat){
    listOfArt = listOfArt.map(x=> x = [x.split(' ')[0].split('')[0] , Number(x.split(' ')[1])])
    listOfArt = listOfArt.filter(x => listOfCat.includes(x[0])  )
     let result = listOfCat.map(x=> x= [x,0]) 
      for(let i = 0 ; i < listOfArt.length ; i ++){
        for(let j = 0 ; j<result.length ; j++){
         if( listOfArt[i][0] == result[j][0] ){
           result[j][1] = result[j][1] + listOfArt[i][1]
         }
        }
      }
      let sumAll = 0
     for(let j = 0 ; j<result.length ; j++){
       sumAll = sumAll + result[j][1]
     }if(sumAll == 0){
       return ""
     }
      let string = ""
      for(let i = 0 ; i < result.length ; i++){
        string = string + "(" + result[i][0] + " : " + result[i][1] + ")" + " - "
       }
      let deleted = string.split("").lastIndexOf("-")
      string = string.split("").filter((x,i)=> i != deleted).join("").trim()
      console.log(string)
        return string
     }
...
(A : 20) - (B : 114) - (C : 50) - (W : 0)
where A, B, C, W are the categories, 20 is the sum of the unique book of category A, 114 the sum corresponding to "BKWRK" and "BTSQZ", 50 corresponding to "CDXEF" and
